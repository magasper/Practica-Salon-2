...
#define nURLStarwarsPeople  @"calendar/2017/11/15"
            ...
            if(data!=nil){
                ...
                if(jsonResponse!=nil){
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        @try {
                            NSError *err = nil;
                            NSDictionary *games = [[jsonResponse objectForKey:@"response"] objectForKey:@"games"];
                            NSString *gamesKey = [[games allKeys] objectAtIndex:0];
                            NSMutableArray *gamesList = [games objectForKey:gamesKey];
                            handler(gamesList);
                        }
                ...
                if(jsonResponse!=nil){
                    ...
                    if(1){
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            ...
                            @try {
                                NSError *err = nil;
                                NSMutableArray<DateGamesModel *>  *people = (NSMutableArray<DateGamesModel *> *)[SWObject arrayOfModelsFromDictionaries:[[[jsonResponse objectForKey:@"response"] objectForKey:@"games"]objectAtIndex:0] error:&err];
                                handler(people);
                            }
                            @catch (NSException *exception) {
                                handler(nil);
